...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	shopCredentialEventOutboxRepository := repository.NewShopCredentialEventOutboxRepository(db)
	shopCredentialEventWorker, err := service.ProvideShopCredentialEventWorker(shopCredentialEventOutboxRepository, configConfig)
	if err != nil {
		return nil, err
	}
	opsService := service.ProvideOpsService(opsRepository, settingRepository, configConfig, accountRepository, userRepository, concurrencyService, gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, opsSystemLogSink, settingService, authCacheInvalidationWorker, shopCredentialEventWorker, apiKeyService)
	usageHandler := handler.NewUsageHandler(usageService, apiKeyService, opsService, settingService)
	lobeConfigService := service.NewLobeConfigService(apiKeyService, gatewayService, configConfig)
	chatHandler := handler.NewChatHandler(configConfig, authService, userService, settingService, lobeConfigService)
	lobeConfigHandler := handler.NewLobeConfigHandler(lobeConfigService)
	...
	modelMarketplaceMonitorRepository := repository.NewModelMarketplaceMonitorRepository(client, db)
	modelMarketplaceMonitorService := service.ProvideModelMarketplaceMonitorService(modelMarketplaceMonitorRepository, secretEncryptor, pricingService)
	modelMarketplaceUserHandler := handler.NewModelMarketplaceUserHandler(modelMarketplaceMonitorService)
	...
	adminSubscriptionHandler := admin.NewSubscriptionHandler(subscriptionService, userService)
	...
	modelMarketplaceMonitorHandler := admin.NewModelMarketplaceMonitorHandler(modelMarketplaceMonitorService)
	modelMarketplaceTemplateRepository := repository.NewModelMarketplaceTemplateRepository(client, db)
	modelMarketplaceTemplateService := service.NewModelMarketplaceTemplateService(modelMarketplaceTemplateRepository)
	modelMarketplaceTemplateHandler := admin.NewModelMarketplaceTemplateHandler(modelMarketplaceTemplateService)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, operationHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, auditHandler, refundInspectionHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, modelMarketplaceMonitorHandler, modelMarketplaceTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	globeService := service.ProvideGlobeService(db)
	...
	handlers := handler.ProvideHandlers(authHandler, oidcIssuerHandler, userHandler, apiKeyHandler, usageHandler, chatHandler, lobeConfigHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, modelMarketplaceUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, globeHandler, modelPlazaHandler, asyncImageHandler, batchImageHandler, idempotencyCoordinator, idempotencyCleanupService)
	...
	engine := server.ProvideRouter(configConfig, handlers, jwtAuthMiddleware, optionalJWTAuthMiddleware, adminAuthMiddleware, apiKeyAuthMiddleware, auditLogMiddleware, stepUpAuthMiddleware, apiKeyService, subscriptionService, opsService, settingService, adminAuditService, compositeRouteResolver, redisClient)
	...
	modelMarketplaceMonitorRunner := service.ProvideModelMarketplaceMonitorRunner(modelMarketplaceMonitorService)
	userPlatformQuotaUsageFlusher := service.ProvideUserPlatformQuotaUsageFlusher(configConfig, billingCache, serviceUserPlatformQuotaRepository, timingWheelService)
	v := provideCleanup(client, redisClient, opsMetricsCollector, opsAggregationService, opsAlertEvaluatorService, opsCleanupService, opsScheduledReportService, opsSystemLogSink, opsService, opsIngressRejectAggregator, apiKeyService, authCacheInvalidationWorker, shopCredentialEventWorker, schedulerSnapshotService, tokenRefreshService, accountExpiryService, proxyExpiryService, subscriptionExpiryService, usageCleanupService, idempotencyCleanupService, batchImageCleanupService, batchImageWorkerRuntime, pricingService, emailQueueService, billingCacheService, usageRecordWorkerPool, subscriptionService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, grokOAuthService, openAIGatewayService, scheduledTestRunnerService, backupService, paymentOrderExpiryService, channelMonitorRunner, modelMarketplaceMonitorRunner, userPlatformQuotaUsageFlusher, upstreamBillingProbeService, ollamaCloudUsageService, auditLogService, promptService)
