...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	// “购买订阅”页面配置验证（兼容旧 purchase_subscription_url 字段）。
	...
	validateOptionalURL := func(rawURL, fieldName string) bool {
		if rawURL == "" {
			return true
		}
	...
	if !validateOptionalURL(purchaseEmbeddedURL, "Purchase Subscription Embedded URL") ||
		!validateOptionalURL(purchaseRedirectURL, "Purchase Subscription Redirect URL") {
		return
	}
	...
	if !validateOptionalURL(chatPageURL, "Chat Page URL") || !validateOptionalURL(agentPageURL, "Agent Page URL") {
		return
	}
	if purchaseEnabled {
		if purchaseMode == service.PurchaseSubscriptionModeRedirect && purchaseRedirectURL == "" {
			response.BadRequest(c, "Purchase Subscription Redirect URL is required when redirect mode is enabled")
			return
		}
		if purchaseMode != service.PurchaseSubscriptionModeRedirect && purchaseEmbeddedURL == "" {
	...
	settings := &service.SystemSettings{
		...
		ChatPageEnabled: func() bool {
			if req.ChatPageEnabled != nil {
				return *req.ChatPageEnabled
			}
