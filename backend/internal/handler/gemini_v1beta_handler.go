...
func (h *GatewayHandler) GeminiV1BetaModels(c *gin.Context) {
	...
	geminiConcurrency.SetMaxUserWaiting(h.concurrencyHelper.ConfiguredMaxUserWaiting())
	...
	userReleaseFunc, err := geminiConcurrency.AcquireUserSlotWithAdmission(c, authSubject.UserID, authSubject.Concurrency, stream, &streamStarted)
	if err != nil {
