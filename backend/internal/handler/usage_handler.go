...
func (h *UsageHandler) Stats(c *gin.Context) {
	...
	response.Success(c, dto.UsageStatsFromAggregate(stats))
}
...
func (h *UsageHandler) DashboardSnapshotV2(c *gin.Context) {
	...
	if includeTrend {
		...
		resp["trend"] = dto.UserTrendDataPointsFromService(trend)
	}
	if includeModels {
		...
		resp["models"] = dto.UserModelStatsFromService(models)
	}
	if includeGroups {
		...
		resp["groups"] = dto.UserGroupStatsFromService(groups)
	}
