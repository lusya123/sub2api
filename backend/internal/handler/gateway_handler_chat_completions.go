...
func (h *GatewayHandler) ChatCompletions(c *gin.Context) {
	...
	// 1. Acquire user concurrency slot; only overflow requests enter the bounded wait queue.
	userReleaseFunc, err := h.concurrencyHelper.AcquireUserSlotWithAdmission(c, subject.UserID, subject.Concurrency, reqStream, &streamStarted)
	if err != nil {
	...
	for {
		...
		if !selection.Acquired {
			...
			accountReleaseFunc, err = h.concurrencyHelper.AcquireAccountSlotWithAdmission(
				c,
				account.ID,
