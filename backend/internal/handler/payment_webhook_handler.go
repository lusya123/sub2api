...
// ThirdPayNotify handles ThirdPay payment notifications.
// POST /api/v1/payment/webhook/thirdpay
func (h *PaymentWebhookHandler) ThirdPayNotify(c *gin.Context) {
	h.handleNotify(c, payment.TypeThirdPay)
}
...
func extractOutTradeNo(rawBody, providerKey string) string {
	switch providerKey {
	case payment.TypeEasyPay, payment.TypeThirdPay, payment.TypeAlipay:
