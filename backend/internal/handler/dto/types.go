...
// UsageEndpointStat is the user-safe endpoint breakdown. Standard cost is
// intentionally excluded; aggregate stats can span groups with different
// show_cost_breakdown policies.
...
type UsageStats struct {
	...
	Endpoints                []UsageEndpointStat `json:"endpoints,omitempty"`
}
...
// UserGroupStat is the regular-user group aggregate. It deliberately exposes
// only the amount actually billed; standard and account costs are admin data
// for aggregates that may span records with different visibility snapshots.
