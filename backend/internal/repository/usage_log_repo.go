...
var usageLogInsertArgTypes = [...]string{
	...
	"numeric",     // actual_rate_multiplier
	"boolean",     // show_cost_breakdown
...
func prepareUsageLogInsert(log *service.UsageLog) usageLogInsertPrepared {
	...
	actualRateMultiplier := nullFloat64(log.ActualRateMultiplier)
	showCostBreakdown := nullBoolValue(log.ShowCostBreakdown)
...
func (r *usageLogRepository) GetUserDashboardStats(ctx context.Context, userID int64) (*UserDashboardStats, error) {
	...
	if err := scanSingleRow(
		...
		[]any{userID, service.APIKeyTypeChat},
		&stats.TotalAPIKeys,
	); err != nil {
		return nil, err
	}
	if err := scanSingleRow(
		ctx,
		r.sql,
		"SELECT COUNT(*) FROM api_keys WHERE user_id = $1 AND status = $2 AND type <> $3 AND deleted_at IS NULL",
		[]any{userID, service.StatusActive, service.APIKeyTypeChat},
		&stats.ActiveAPIKeys,
	); err != nil {
		return nil, err
	}
...
func scanUsageLog(scanner interface{ Scan(...any) error }) (*service.UsageLog, error) {
	var (
		...
		actualCost            float64
		rateMultiplier        float64
		actualRateMultiplier  sql.NullFloat64
		showCostBreakdown     sql.NullBool
	...
	log := &service.UsageLog{
		...
		ActualCost:            actualCost,
		RateMultiplier:        rateMultiplier,
		ActualRateMultiplier:  nullFloat64Ptr(actualRateMultiplier),
		ShowCostBreakdown:     nullBoolPtr(showCostBreakdown),
...
func nullBoolValue(v *bool) sql.NullBool {
	if v == nil {
		return sql.NullBool{}
	}
