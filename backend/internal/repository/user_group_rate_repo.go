...
func (r *userGroupRateRepository) GetByGroupID(ctx context.Context, groupID int64) ([]service.UserGroupRateEntry, error) {
	query := `
		SELECT ugr.user_id, u.username, u.email, COALESCE(u.notes, ''), u.status, ugr.rate_multiplier, ugr.actual_rate_multiplier, ugr.rpm_override
	...
	for rows.Next() {
		...
		if err := rows.Scan(&entry.UserID, &entry.UserName, &entry.UserEmail, &entry.UserNotes, &entry.UserStatus, &rate, &actualRate, &rpm); err != nil {
			return nil, err
		}
		if rate.Valid {
			v := rate.Float64
		...
		if actualRate.Valid {
			v := actualRate.Float64
			entry.ActualRateMultiplier = &v
		}
		if rpm.Valid {
			v := int(rpm.Int32)
			entry.RPMOverride = &v
...
// GetRateConfigByUserAndGroup 获取用户在特定分组的展示/实际倍率配置。
func (r *userGroupRateRepository) GetRateConfigByUserAndGroup(ctx context.Context, userID, groupID int64) (*service.UserGroupRateConfig, error) {
	...
	var rate sql.NullFloat64
	var actualRate sql.NullFloat64
	err := scanSingleRow(ctx, r.sql, query, []any{userID, groupID}, &rate, &actualRate)
	...
	cfg := &service.UserGroupRateConfig{RateMultiplier: rate.Float64}
	if actualRate.Valid {
		v := actualRate.Float64
		cfg.ActualRateMultiplier = &v
	}
	return cfg, nil
}
...
func (r *userGroupRateRepository) SyncUserGroupRates(ctx context.Context, userID int64, rates map[int64]*float64) error {
	if len(rates) == 0 {
		if _, err := r.sql.ExecContext(ctx, `
			...
			SET rate_multiplier = NULL, actual_rate_multiplier = NULL, updated_at = NOW()
			WHERE user_id = $1
		`, userID); err != nil {
			return err
		}
		_, err := r.sql.ExecContext(ctx,
			`DELETE FROM user_group_rate_multipliers WHERE user_id = $1 AND rate_multiplier IS NULL AND actual_rate_multiplier IS NULL AND rpm_override IS NULL`,
			userID)
		return err
	}
	...
	if len(clearGroupIDs) > 0 {
		...
		if _, err := r.sql.ExecContext(ctx,
			`DELETE FROM user_group_rate_multipliers WHERE user_id = $1 AND group_id = ANY($2) AND rate_multiplier IS NULL AND actual_rate_multiplier IS NULL AND rpm_override IS NULL`,
			userID, pq.Array(clearGroupIDs)); err != nil {
			return err
		}
	}
...
func (r *userGroupRateRepository) SyncGroupRateMultipliers(ctx context.Context, groupID int64, entries []service.GroupRateMultiplierInput) error {
	...
	if _, err := r.sql.ExecContext(ctx, `
		...
		WHERE group_id = $1 AND rate_multiplier IS NULL AND actual_rate_multiplier IS NULL AND rpm_override IS NULL
	`, groupID); err != nil {
		return err
	}
