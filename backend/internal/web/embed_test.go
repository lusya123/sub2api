...
func TestFrontendServer_ServeIndexHTML(t *testing.T) {
	...
	t.Run("sets_cache_control_header", func(t *testing.T) {
		...
		assert.Equal(t, "public, max-age=30, s-maxage=300, stale-while-revalidate=60", w.Header().Get("Cache-Control"))
	})
...
func TestFrontendServer_Middleware(t *testing.T) {
	...
	t.Run("returns_404_for_missing_assets", func(t *testing.T) {
		provider := &mockSettingsProvider{
			settings: map[string]string{"test": "value"},
		}
