...
func (s *OpenAIGatewayService) resolveUserGroupActualRateMultiplier(ctx context.Context, userID, groupID int64, groupActualMultiplier float64) float64 {
	if s == nil {
...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	if account != nil && account.IsVirtualCacheEnabled() {
	...
	if apiKey.GroupID != nil && apiKey.Group != nil {
		...
		actualMultiplier = s.resolveUserGroupActualRateMultiplier(ctx, user.ID, *apiKey.GroupID, apiKey.Group.EffectiveActualRateMultiplier())
	}
	// token 倍率叠加高峰因子（token 计费含图片 token，图片按次倍率不受影响）。高峰因子按请求时刻现算，
	// 不并入上面的 Resolve，以免污染 user:group 倍率缓存。
	baseMultiplier := multiplier
	baseActualMultiplier := actualMultiplier
	now := timezone.Now()
	multiplier, imageMultiplier := computePeakAwareMultipliers(apiKey, baseMultiplier, now)
	actualMultiplier, imageActualMultiplier := computePeakAwareMultipliers(apiKey, baseActualMultiplier, now)
	videoMultiplier := resolveVideoRateMultiplier(apiKey, baseMultiplier)
	videoActualMultiplier := resolveVideoRateMultiplier(apiKey, baseActualMultiplier)
	...
	cost, err = s.calculateOpenAIRecordUsageCost(
		...
		billingModels,
		actualMultiplier,
		imageActualMultiplier,
		videoActualMultiplier,
		baseActualMultiplier,
		tokens,
		serviceTier,
	...
	if isVideoUsage && (cost == nil || cost.BillingMode != string(BillingModeToken)) {
		...
		usageLog.ActualRateMultiplier = &videoActualMultiplier
	...
	usageLog.ShowCostBreakdown = SnapshotShowCostBreakdown(apiKey.Group)
