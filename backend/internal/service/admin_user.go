...
// UpdateUserRole preserves the dedicated user/operator role-management API.
// Super-admin accounts are intentionally immutable through this narrower path.
func (s *adminServiceImpl) UpdateUserRole(ctx context.Context, id int64, role string) (*User, error) {
	role = strings.TrimSpace(role)
	if role != RoleUser && role != RoleOperator {
		return nil, fmt.Errorf("role must be one of: %s/%s", RoleUser, RoleOperator)
	}
	user, err := s.userRepo.GetByID(ctx, id)
	if err != nil {
		return nil, err
