...
type AnthropicEventToResponsesState struct {
	...
	// For reasoning output: accumulate summary text
	CurrentReasoning string

	// Completed output items for the terminal response.completed payload.
...
func anthToResHandleContentBlockDelta(evt *AnthropicStreamEvent, state *AnthropicEventToResponsesState) []ResponsesStreamEvent {
	...
	case "thinking_delta":
		...
		state.CurrentReasoning += evt.Delta.Thinking
