...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		registerAccountBridgeRoutes(admin, h)
...
func registerAuditLogRoutes(admin *gin.RouterGroup, h *handler.Handlers, _ middleware.StepUpAuthMiddleware) {
	securityAuditLogs := admin.Group("/security-audit-logs")
	{
		securityAuditLogs.GET("", h.Admin.AuditLog.List)
		securityAuditLogs.GET("/:id", h.Admin.AuditLog.Get)
		// 清空需现场 TOTP 校验（在 handler 内强制），不复用 step-up sudo 窗口
		securityAuditLogs.POST("/clear", h.Admin.AuditLog.Clear)
	}

	// 兼容旧版控制台的清空请求。旧 GET 契约不保留，避免与上游管理活动审计冲突。
	admin.POST("/audit-logs/clear", h.Admin.AuditLog.Clear)
}
