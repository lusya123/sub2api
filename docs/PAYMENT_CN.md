...
| **ThirdPay（第三方聚合支付）** | 支付宝、微信支付 | 兼容 Dujiao-Next 的 ThirdPay 第三方聚合支付协议 |
...
> 支付宝官方 / 微信官方、易支付、ThirdPay 可以同时作为后台服务商实例存在，但前台始终只展示 `支付宝`、`微信支付` 两个可见按钮。管理员需要分别为这两个按钮选择唯一支付来源：官方、易支付或 ThirdPay。
...
- **支付宝**：后台启用后，需要额外指定该按钮路由到 `支付宝官方`、`易支付支付宝` 或 `ThirdPay 支付宝`
- **微信支付**：后台启用后，需要额外指定该按钮路由到 `微信官方`、`易支付微信` 或 `ThirdPay 微信`
...
兼容 Dujiao-Next 使用的第三方聚合支付协议（`mch_id`、`reserve`、`create_order_url`）。

| 参数 | 说明 | 必填 |
|------|------|------|
| **创建订单接口** | ThirdPay 创建订单接口 | 是 |
| **查询订单接口** | ThirdPay 查单接口，用于订单过期前补偿查询 | 否 |
| **商户 ID** | ThirdPay 商户号（`mch_id`） | 是 |
| **商户密钥** | MD5 签名密钥 | 是 |
| **Reserve 请求头** | 网关要求的 `reserve` 请求头 | 是 |
| **支付宝类型值** | 网关内支付宝支付类型值，默认 `2` | 否 |
| **微信类型值** | 网关内微信支付类型值，默认 `1` | 否 |
| **商品描述** | 可选的短商品描述 | 否 |
