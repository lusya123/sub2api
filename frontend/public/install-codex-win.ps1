...
function Install-CodexCli {
  ...
  Write-XdtLog 'Installing Codex CLI with npm'
  & npm install -g '@openai/codex' --registry $registry
  ...
  if ($LASTEXITCODE -ne 0) {
    Fail-Xdt "Codex CLI npm installation failed with exit code $LASTEXITCODE"
  }
  Refresh-ProcessPath
}
