const custom = {
  ...
  "modelMarketplaceStatus": {
    "title": "模型状态",
    "description": "查看受监控模型的可用性、延迟和近期状态",
    "priceDisclaimer": "本页价格和倍率仅用于监控展示，不参与实际结算；最终费用以使用记录或账单为准。",
    ...
    "allFilters": "展示“模型监控配置”中已启用的全部模型",
    "allProviders": "全部供应商",
    "allStatuses": "全部状态",
    "primaryModel": "主模型",
    "extraModel": "附属模型",
    ...
    "ratedPrice": "倍率 ×{rate}",
    "discount": "展示倍率",
    "channelFinalPrice": "监控展示价",
    "officialDiscount": "参考系数",
    "officialPrice": "官方参考价",
    "exchangeDiscount": "汇率换算系数",
    "officialInputPrice": "参考输入价",
    "officialOutputPrice": "参考输出价",
    "officialRequestPrice": "参考请求价",
    "officialImagePrice": "参考图片价",
    "officialDiscountValue": "参考系数 {discount}/10",
    "exchangeDiscountValue": "汇率换算系数 {discount}/10",
    "rateDiscountValue": "展示倍率 {discount}/10",
    "finalDiscountValue": "综合展示系数 {discount}/10",
    "exchangeRateHint": "按 $1≈¥{rate}",
    "officialSavingValue": "较参考价低 {saving}%",
    "priceUnavailable": "暂无参考价",
    "callModel": "调用模型",
    "requestUrl": "请求地址",
  ...
  "admin": {
    ...
    "modelMarketplaceMonitor": {
      "title": "模型监控配置",
      "description": "维护模型状态页的可用性、延迟和展示内容",
      ...
      "effectiveRateRequired": "请输入大于 0 的监控展示倍率",
      "invalidPricing": "请输入大于或等于 0 的模型参考原价",
      "columns": {
        "name": "名称",
        "provider": "供应商",
        "primaryModel": "主模型",
        "effectiveRate": "监控展示倍率",
        "availability7d": "7 天可用率",
        "latency": "延迟 (ms)",
        "enabled": "启用",
        "actions": "操作"
      ...
      "form": {
        ...
        "displayNameHint": "展示名只影响模型状态页面。中文界面优先显示中文名，没有中文名时回退英文名。未配置展示名时，用户侧不显示模型标题。",
        "callModel": "完整调用模型名",
        "requestUrl": "完整请求地址",
        "callConfigHint": "调用模型名和请求地址会展示给用户用于复制配置；留空时使用模型 ID，并按当前项目的 OpenAI/Anthropic/Gemini 默认入口推导请求地址。同一个完整调用模型名可以配置多条监控渠道，用户侧会自动合并展示每条渠道的展示倍率、参考价和健康条。",
        "customPricing": "自定义模型参考原价",
        "customPricingHint": "按每 1M token 的美元价格填写。留空时继续使用内置/New API 默认价格；填写后模型状态页的参考原价和监控展示价都会按这里计算，不影响用户实际结算。",
        ...
        "effectiveRateHint": "仅影响模型状态页的参考价和展示系数，默认 1；不是用户实际结算倍率，也不会读取分组管理里的倍率。最终费用以使用记录或账单为准。",
    ...
    "settings": {
      "features": {
        "channelMonitor": {
          "modelMarketplaceEnabled": "向普通用户显示模型状态",
          "modelMarketplaceEnabledHint": "关闭后顶部/底部“模型状态”入口隐藏，用户直接访问页面或接口也会被拦截。"
        },
        "chatPage": {
          "title": "用 Token 应用",
