    ...
    <div class="purchase-page-layout">
      <div class="card min-h-0 flex-1 overflow-hidden">
        ...
        <div v-else class="purchase-embed-shell">
          ...
          <iframe :src="embeddedUrl" class="purchase-embed-frame" allowfullscreen></iframe>
        </div>
      </div>
    </div>
  </AppLayout>
...
const purchaseMode = computed(() =>
  normalizePurchaseSubscriptionMode(
    appStore.cachedPublicSettings?.purchase_subscription_mode,
  ),
);
...
const isConfigured = computed(() => {
  const targetUrl =
    purchaseMode.value === "redirect" ? redirectUrl.value : embeddedUrl.value;
