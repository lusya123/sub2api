...
export async function list(params: AuditLogQuery): Promise<AuditLogListResponse> {
  const { data } = await apiClient.get('/admin/security-audit-logs', { params })
  return data
}
...
export async function get(id: number): Promise<AuditLog> {
  const { data } = await apiClient.get(`/admin/security-audit-logs/${id}`)
  return data
}
...
export async function clear(totpCode: string): Promise<{ deleted: number }> {
  const { data } = await apiClient.post('/admin/security-audit-logs/clear', { totp_code: totpCode })
  return data
}
