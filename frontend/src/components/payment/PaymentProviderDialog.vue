...
const providerWebhookHintMap: Record<string, string> = {
  thirdpay: 'admin.settings.payment.thirdpayWebhookHint',
...
const supportsRefund = computed(() => form.provider_key !== 'thirdpay')
...
function onKeyChange() {
  ...
  normalizeRefundSettings()
  clearConfig()
  applyDefaults()
}

function normalizeRefundSettings() {
...
function handleSave() {
  ...
  const refundEnabled = supportsRefund.value && form.refund_enabled
  emit('save', {
    ...
    allow_user_refund: refundEnabled ? form.allow_user_refund : false,
...
function loadProvider(provider: ProviderInstance) {
  ...
  form.allow_user_refund = form.refund_enabled ? provider.allow_user_refund : false
  clearConfig()
