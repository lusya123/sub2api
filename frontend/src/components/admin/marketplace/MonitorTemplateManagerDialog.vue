<template>
  <BaseDialog
    :show="show"
    :title="t('admin.modelMarketplaceMonitor.template.managerTitle')"
    width="wide"
    @close="$emit('close')"
  >
    <!-- provider tabs -->
    ...
    <div v-if="!editing" class="space-y-2">
      <div class="flex justify-end">
        <button class="btn btn-primary btn-sm" @click="openCreateForm">
          ...
          {{ t('admin.modelMarketplaceMonitor.template.createButton') }}
        </button>
      </div>
      ...
      >
        {{ t('admin.modelMarketplaceMonitor.template.emptyState') }}
      </div>
        ...
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0 flex-1">
            <div class="flex items-center gap-2">
              ...
              >
                {{ t('admin.modelMarketplaceMonitor.template.associatedCount', { n: tpl.associated_monitors }) }}
              </span>
            </div>
            ...
            <p class="mt-1 text-xs text-gray-400">
              {{ t('admin.modelMarketplaceMonitor.template.headersSummary', {
          ...
          <div class="flex flex-shrink-0 gap-2">
            <button
              ...
              :title="t('admin.modelMarketplaceMonitor.template.applyTooltip')"
              @click="confirmApply(tpl)"
            >
              <Icon name="refresh" size="sm" class="mr-1" />
              {{ t('admin.modelMarketplaceMonitor.template.applyButton') }}
            </button>
    ...
    <div v-else class="space-y-4">
      <div>
        <label class="input-label">
          {{ t('admin.modelMarketplaceMonitor.template.form.name') }}
        ...
        <input
          ...
          required
          class="input"
          :placeholder="t('admin.modelMarketplaceMonitor.template.form.namePlaceholder')"
        />
      </div>

      <div v-if="editing === 'new'">
        <label class="input-label">
          {{ t('admin.modelMarketplaceMonitor.form.provider') }}
          <span class="text-red-500">*</span>
        </label>
      ...
      <div>
        <label class="input-label">
          {{ t('admin.modelMarketplaceMonitor.template.form.description') }}
        </label>
        <input
          v-model="form.description"
          type="text"
          class="input"
          :placeholder="t('admin.modelMarketplaceMonitor.template.form.descriptionPlaceholder')"
        />
      </div>
...
import MonitorTemplateApplyPickerDialog from '@/components/admin/marketplace/MonitorTemplateApplyPickerDialog.vue'
...
const templates = ref<ModelMarketplaceTemplate[]>([])
...
function loadForm(tpl: ModelMarketplaceTemplate) {
  form.id = tpl.id
  form.name = tpl.name
...
function openEditForm(tpl: ModelMarketplaceTemplate) {
  loadForm(tpl)
...
async function handleSubmit() {
  ...
  if (!form.name.trim()) {
    appStore.showError(t('admin.modelMarketplaceMonitor.template.missingName'))
    return
  }
  submitting.value = true
  try {
    if (editing.value === 'new') {
      await adminAPI.modelMarketplaceTemplate.create({
        name: form.name.trim(),
      ...
      appStore.showSuccess(t('admin.modelMarketplaceMonitor.template.createSuccess'))
    } else if (typeof editing.value === 'number') {
      await adminAPI.modelMarketplaceTemplate.update(editing.value, {
        name: form.name.trim(),
        description: form.description.trim(),
        extra_headers: form.extra_headers,
        body_override_mode: form.body_override_mode,
      ...
      appStore.showSuccess(t('admin.modelMarketplaceMonitor.template.updateSuccess'))
    }
...
const applyPicker = reactive<{ show: boolean; tpl: ModelMarketplaceTemplate | null }>({
  show: false,
  tpl: null,
})

function confirmApply(tpl: ModelMarketplaceTemplate) {
  applyPicker.tpl = tpl
...
// --- delete ---
const confirmDelete = reactive<{ show: boolean; tpl: ModelMarketplaceTemplate | null }>({
  show: false,
  tpl: null,
})

function handleDelete(tpl: ModelMarketplaceTemplate) {
...
const confirmDeleteMessage = computed(() => {
  ...
  if (!tpl) return ''
  return t('admin.modelMarketplaceMonitor.template.deleteConfirm', {
    name: tpl.name,
...
async function doDelete() {
  ...
  if (!tpl) return
  try {
    await adminAPI.modelMarketplaceTemplate.del(tpl.id)
    appStore.showSuccess(t('admin.modelMarketplaceMonitor.template.deleteSuccess'))
    await fetchTemplates()
    emit('updated')
  } catch (err: unknown) {
    appStore.showError(extractApiErrorMessage(err, t('common.error')))
...
function modeLabel(mode: BodyOverrideMode): string {
  return t(`admin.modelMarketplaceMonitor.advanced.bodyMode${mode.charAt(0).toUpperCase()}${mode.slice(1)}`)
}
</script>
