<template>
  <BaseDialog
    :show="show"
    :title="t('admin.modelMarketplaceMonitor.form.selectKeyTitle')"
    width="wide"
    @close="$emit('close')"
  >
    <div class="space-y-3">
      <p class="text-xs text-gray-500 dark:text-gray-400">
        {{ t('admin.modelMarketplaceMonitor.form.selectKeyHint') }}
      </p>
      ...
      <div v-else-if="filteredKeys.length === 0" class="py-6 text-center text-sm text-gray-500">
        {{ t('admin.modelMarketplaceMonitor.form.noActiveKey') }}
      </div>
