<template>
  <BaseDialog
    :show="show"
    :title="t('admin.modelMarketplaceMonitor.template.applyPickerTitle', { name: templateName })"
    @close="$emit('close')"
  >
    <p class="mb-3 text-sm text-gray-600 dark:text-gray-400">
      {{ t('admin.modelMarketplaceMonitor.template.applyPickerHint') }}
    </p>
    ...
    <div v-else-if="monitors.length === 0" class="py-6 text-center text-sm text-gray-400">
      {{ t('admin.modelMarketplaceMonitor.template.applyPickerEmpty') }}
    </div>
    ...
    <div v-else>
      ...
      <div class="mb-2 flex items-center gap-3 text-xs">
        ...
        <button
          ...
          @click="selectNone"
        >
          {{ t('admin.modelMarketplaceMonitor.template.selectNone') }}
        </button>
        <span class="ml-auto text-gray-500 dark:text-gray-400">
          {{ t('admin.modelMarketplaceMonitor.template.selectedCount', {
      ...
      <ul class="max-h-80 divide-y divide-gray-100 overflow-y-auto rounded-lg border border-gray-200 dark:divide-dark-700 dark:border-dark-700">
        ...
        >
          ...
          >
            {{ t('admin.modelMarketplaceMonitor.onlyDisabled').replace(/^仅|^Only /, '') }}
          </span>
        </li>
      </ul>
    </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        ...
        >
          {{ submitting
            ? t('common.submitting')
            : t('admin.modelMarketplaceMonitor.template.applyPickerConfirm', { n: selectedIds.length }) }}
        </button>
      </div>
    </template>
  </BaseDialog>
...
import type { AssociatedMonitorBrief } from '@/api/admin/modelMarketplaceTemplate'
...
async function fetchMonitors(id: number) {
  ...
  selectedIds.value = []
  try {
    const { items } = await adminAPI.modelMarketplaceTemplate.listAssociatedMonitors(id)
...
async function handleApply() {
  ...
  try {
    const { affected } = await adminAPI.modelMarketplaceTemplate.apply(
      props.templateId,
      [...selectedIds.value],
    )
    appStore.showSuccess(t('admin.modelMarketplaceMonitor.template.applySuccess', { n: affected }))
