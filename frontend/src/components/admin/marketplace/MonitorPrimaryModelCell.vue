          ...
          <table class="w-full text-left text-[11px]">
            <thead>
              <tr class="text-gray-400">
                <th class="py-0.5 pr-2 font-medium">{{ t('admin.modelMarketplaceMonitor.columns.primaryModel') }}</th>
                <th class="py-0.5 pr-2 font-medium">{{ t('admin.modelMarketplaceMonitor.columns.actions') }}</th>
                <th class="py-0.5 font-medium">{{ t('admin.modelMarketplaceMonitor.columns.latency') }}</th>
              </tr>
            </thead>
            <tbody>
...
import type { ModelMarketplaceMonitor } from '@/api/admin/modelMarketplaceMonitor'
import HelpTooltip from '@/components/common/HelpTooltip.vue'
import { useModelMarketplaceMonitorFormat } from '@/composables/useModelMarketplaceMonitorFormat'

defineProps<{
  row: ModelMarketplaceMonitor
}>()

const { t } = useI18n()
const { statusLabel, statusBadgeClass, formatLatency } = useModelMarketplaceMonitorFormat()
</script>
