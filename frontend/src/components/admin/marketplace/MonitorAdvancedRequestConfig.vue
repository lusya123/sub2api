<template>
  <div class="space-y-4">
    ...
    <div>
      <label class="input-label">{{ t('admin.modelMarketplaceMonitor.advanced.headers') }}</label>
      <div class="space-y-1.5">
        ...
        >
          <input
            ...
            type="text"
            spellcheck="false"
            :placeholder="t('admin.modelMarketplaceMonitor.advanced.headerNamePlaceholder')"
          ...
          <input
            ...
            type="text"
            spellcheck="false"
            :placeholder="t('admin.modelMarketplaceMonitor.advanced.headerValuePlaceholder')"
        ...
        >
          ...
          {{ t('admin.modelMarketplaceMonitor.advanced.headerAddRow') }}
        </button>
      </div>
      <p v-if="headersError" class="mt-1 text-xs text-red-500">{{ headersError }}</p>
      <p v-else class="mt-1 text-xs text-gray-400">
        {{ t('admin.modelMarketplaceMonitor.advanced.headersHint') }}
      </p>
    </div>

    <!-- Body mode radio -->
    <div>
      <label class="input-label">{{ t('admin.modelMarketplaceMonitor.advanced.bodyMode') }}</label>
    ...
    <div v-if="bodyOverrideMode !== 'off'">
      <div class="mb-1 flex items-center justify-between">
        <label class="input-label !mb-0">{{ t('admin.modelMarketplaceMonitor.advanced.bodyJson') }}</label>
        <button
          type="button"
          ...
          @click="formatBody"
        >
          {{ t('admin.modelMarketplaceMonitor.advanced.bodyJsonFormat') }}
        </button>
      </div>
      <textarea
        v-model="bodyText"
      ...
      <p v-else class="mt-1 text-xs text-gray-400">
        {{ t('admin.modelMarketplaceMonitor.advanced.bodyJsonHint') }}
      </p>
    </div>
  </div>
</template>
...
import type { BodyOverrideMode } from '@/api/admin/modelMarketplaceMonitor'
...
function commitHeaders() {
  ...
  for (const row of headerRows.value) {
    ...
    if (name.includes(':') || /\s/.test(name)) {
      headersError.value = t('admin.modelMarketplaceMonitor.advanced.headerNameInvalid', { name })
      return
    }
  }
  headersError.value = ''
...
function commitBody() {
  ...
  try {
    ...
    if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
      bodyError.value = t('admin.modelMarketplaceMonitor.advanced.bodyJsonObjectError')
      return
    }
    ...
    bodyError.value =
      t('admin.modelMarketplaceMonitor.advanced.bodyJsonError') +
      ': ' +
...
const bodyModeOptions = computed<{ value: BodyOverrideMode; label: string }[]>(() => [
  { value: 'off', label: t('admin.modelMarketplaceMonitor.advanced.bodyModeOff') },
  { value: 'merge', label: t('admin.modelMarketplaceMonitor.advanced.bodyModeMerge') },
  { value: 'replace', label: t('admin.modelMarketplaceMonitor.advanced.bodyModeReplace') },
])
...
const bodyModeHint = computed(() => {
  switch (props.bodyOverrideMode) {
    case 'merge':
      return t('admin.modelMarketplaceMonitor.advanced.bodyModeHintMerge')
    case 'replace':
      return t('admin.modelMarketplaceMonitor.advanced.bodyModeHintReplace')
    default:
      return t('admin.modelMarketplaceMonitor.advanced.bodyModeHintOff')
  }
})
