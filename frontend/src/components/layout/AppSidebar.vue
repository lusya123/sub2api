...
const { canUseBatchImage, ensureBatchImageAccess } = useBatchImageAccess()
...
const purchaseRedirectUrl = computed(() => getPurchaseRedirectUrl(appStore.cachedPublicSettings))
...
const flagPurchase = () => {
  const payment = flagPayment()
  const externalPurchase = appStore.cachedPublicSettings?.purchase_subscription_enabled
...
const adminNavItems = computed((): NavItem[] => {
  const baseItems: NavItem[] = [
    ...
    { path: '/admin/globe', label: t('nav.liveGlobe'), icon: GlobeIcon },
    ...
    { path: '/admin/audit-logs', label: t('nav.operationAuditLogs'), icon: ShieldIcon, hideInSimpleMode: true },
    { path: '/admin/security-audit-logs', label: t('nav.securityAuditLogs'), icon: ShieldIcon, hideInSimpleMode: true }
  ]
